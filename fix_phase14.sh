#!/bin/bash
# ============================================================
# Ad-Leak-Engine [SEED: 2399] — PHASE 15: BULLETPROOF UPGRADE
# Fixes bash truncation + applies camoufox stealth to L2/L3 probes
# ============================================================
set -e
...
# Write the patch logic to a pure Python file to avoid ALL bash escaping issues
cat << 'PYEOF' > scripts/apply_patch.py
import sys
from pathlib import Path

cli_py = """\"\"\"Ad-Leak-Engine Master CLI [SEED: 2399].\"\"\"
import sys
import os
from pathlib import Path
...
l2_probe_py = """\"\"\"REAL Playwright probe using camoufox (stealth) [SEED: 2399].\"\"\"
...
perf_py = """\"\"\"REAL Core Web Vitals measurement via camoufox [SEED: 2399].\"\"\"
from ..self_heal.human_browser import HumanBrowser
...
ux_py = """\"\"\"REAL mobile UX analysis via camoufox [SEED: 2399].\"\"\"
from ..self_heal.human_browser import HumanBrowser
...
template_j2 = """# Ad Infrastructure Teardown: {{ page.name }}
